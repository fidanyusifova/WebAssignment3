...
const Cards = () => {
  ...
  const fetchInitialCards = useCallback(async () => {
    try {
      const response = await axios.get(
        `http://localhost:3001/cards?_page=${page}&_limit=6`
      );
  ...
  const handleShare = () => {
    const selectedCardDetails = cards
      ...
      .map(({ id, frontText, backAnswer, image }) => ({
        id,
        frontText,
        backAnswer,
        image,
    ...
    const mailtoLink = `mailto:?subject=Flash Cards&body=${encodeURIComponent(
      jsonData
    )}`;
  ...
  const handleRearrangeCards = (draggedCardId, dropTargetCardId) => {
    ...
    const dropTargetIndex = cards.findIndex(
      (card) => card.id === dropTargetCardId
    );

    // Swap the positions of the cards
    const newCards = [...cards];
    [newCards[draggedIndex], newCards[dropTargetIndex]] = [
      newCards[dropTargetIndex],
      ...
      try {
        ...
        selectedSortings.forEach((sortingOption) => {
          switch (sortingOption) {
            ...
            case "frontTextZA":
              sortedCards = sortedCards.sort((a, b) =>
                b.frontText.localeCompare(a.frontText)
              );
              break;
            case "backAnswerAZ":
              sortedCards = sortedCards.sort((a, b) =>
                a.backAnswer.localeCompare(b.backAnswer)
              );
              break;
            case "backAnswerZA":
              sortedCards = sortedCards.sort((a, b) =>
                b.backAnswer.localeCompare(a.backAnswer)
              );
              break;
            case "idDescending":
  ...
  return (
    <div>
      ...
      <div className="cards-location">
        <h1 className="text-center">Flash Cards</h1>
        ...
        <InfiniteScroll
          ...
          endMessage={
            <p className="finished-loading">No more cards to load.</p>
          }
        >
